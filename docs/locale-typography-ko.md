...
| **IBM Plex Sans KR** | SIL OFL 1.1 | enterprise(IBM Plex Sans)의 **동일 패밀리** 한글판 | `font-ibm-plex-sans-kr` | ✅ recipe 지명 (P4 승격 2026-07-02) |
| **SUIT** | SIL OFL 1.1 | 기하 성향 — creative-multiscale(Lexend) 정합 | `font-suit` | ✅ recipe 지명 (P4 승격 2026-07-02) |
| Gowun Batang | SIL OFL 1.1 | 감성 세리프 — pro-emotive 대안 | `font-gowun-batang` | 설치 |
| Gowun Dodum | SIL OFL 1.1 | 부드러운 고딕 | `font-gowun-dodum` | 설치 — warm-creator 승격 **기각**(2026-07-02): 단일 웨이트(400)라 제목 볼드 위계 손실 |
| **NanumSquare Round** | OFL 계열(나눔) | 라운드+멀티웨이트 — warm-creator(Nunito) 정합 | `font-nanum-square-round` | ✅ recipe 지명 (P4 승격 2026-07-02). ⚠️ CSS 패밀리명은 공백 없는 **`NanumSquareRound`** (공백 표기는 미매칭 → serif 폴백, 실측) |
...
| 본문 행간 | 1.6–1.8 (생성기 ko 경로에서 `max(1.7, <body token>)` 플로어) | 한글 음절 블록이 em box를 채워 라틴보다 넉넉한 행간 필요(업계 관례: 네이버·카카오 가이드 160~180%) | ✅ 생성기 ko 경로 |
...
| 한 줄 분량(본문) | 공백 포함 **25–35자**(모바일) / **35–45자**(데스크톱) — 생성기 ko 경로에서 본문 copy `min(<latin ch>, 35em)` | 한글 가독성 관례(전각 기준, 라틴 45–75자의 절반 수준) | ✅ 생성기 ko 경로 |
| 한 줄 분량(제목) | 8–15자 내외 — 생성기 ko 경로에서 hero headline `min(<latin ch>, 15em)` | 히어로 헤드라인 관례 | ✅ 생성기 ko 경로 |
| 금칙 처리 | 행두 닫는 괄호·마침표 금지 등 | KLREQ §line-adjustment — 브라우저 기본 처리에 위임 | ✅ (별도 구현 불요) |

### 백로그 티켓 (다음 increment)

1. ✅ **shipped — ko measure 보정**: demo/styleguide의 `Nch` max-width를 ko에서 자수 기반으로
   보정(전각 1em ≈ 1자 → `min(18ch, 15em)` 류). 위 분량표가 수용 기준.
2. ✅ **shipped — 본문 행간 ko 보정**: 토큰 오염 없이 생성기 ko 경로에서 `line-height: 1.7` 플로어.
3. ~~지명 승격~~ — ✅ shipped (2026-07-02, add-typography-foundation P4):
   enterprise→IBM Plex Sans KR, creative-multiscale→SUIT,
   warm-creator→NanumSquareRound (Gowun Dodum은 단일 웨이트로 기각, 육안 비교 실측).
