...
- [ ] 1.1 `golden/video.test.ts` — G-V0 keystone: after the schema type edit,
      `computeTokenHash(build(minimal-tech)) === computeTokenHash(SAMPLE)`
      (proves the optional `video` contract key is hash-neutral)
- [ ] 1.2 G-V1 color: every color path realizes to `#rrggbb`; independent-path
...
- [ ] 1.3 G-V2 numerics: `primitive.space.xs` (abstract 0.5, base 16) → 8;
      durations are plain ms numbers; fontWeight/number pass through
- [ ] 1.4 G-V3 filtering + skip accounting (synthetic case): inject
...
- [ ] 1.5 G-V4 emitted text: `export const tokens`, `export const fontAssets`,
...
- [ ] 1.6 G-V5 toFrames semantics (module fn): 1000ms@30fps=30;
      100ms@30fps round=3; floor=3; ceil=3; 50ms@30fps round=2/floor=1/ceil=2
- [ ] 1.7 G-V6 parity: video-exportable path set == web path set minus
      skipped types minus `target-only:web` (no silent drops)

## 2. Implementation (GREEN)

- [ ] 2.1 `src/tokens-schema.ts` — optional `video` key on `TransformContract`
- [ ] 2.2 `src/color.ts` — `linearToHex(rgb)` (gamma encode + clamp) and
      `toHexColor(s)` (oklch/hex string → `#rrggbb`, throws on unparseable)
- [ ] 2.3 `src/transformer.ts` — `toRealizedVideo(doc)` returning
      `{ values: Map<string, string | number | readonly string[]>, skipped: string[] }`
- [ ] 2.4 `src/adapters/video-adapter.ts` — `toTokensTs(doc)` (header with
      tokenHash + skip accounting, nested `as const` tree, `fontAssets`,
      embedded `toFrames` from single source) + exported `toFrames`
- [ ] 2.5 `src/index.ts` re-export; `src/cli.ts` build writes `tokens.ts`

## 3. Verification

- [ ] 3.1 `npm test` (204 + G-V all green) + `npm run typecheck`
- [ ] 3.2 Synthetic end-to-end: generate `tokens.ts` for one recipe via CLI,
