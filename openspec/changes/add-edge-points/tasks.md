...
- [ ] 1.1 `src/brand-schema.ts`: `edges?: readonly string[]` + EDGE enum
...
- [ ] 2.1 `src/edge-point.ts` (new): `EDGE_MENU` (fitness predicates +
      rationale templates + deferred flag), `suggestEdges(brand, recipe)`.
      Pure/deterministic — no Date/random, stable ordering.
- [ ] 2.2 Fitness rules v1 exactly as proposal.md (texture-grain:
...
- [ ] 5.2 No-edge byte-identity: keystone + full existing suite pass
      **unmodified** (do not regenerate keystone; if it changes, the
      implementation is wrong).
- [ ] 5.3 `npm test` green (288 existing + new), `npx tsc --noEmit` clean.
