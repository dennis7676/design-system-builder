...
function collageDemoCss(doc: TokensDocument, tier: DemoTier, ko: boolean): string {
  ...
  return `${toCssVars(doc)}
    * { box-sizing: border-box; }
    ...
    .eyebrow { font-family: var(--primitive-font-family-mono, ui-monospace, monospace); text-transform: uppercase; letter-spacing: .08em; font-size: .78rem; color: ${textMix(62, "collage.eyebrow")}; }
    .lead { max-width: 52ch; color: ${textMix(76, "collage.lead")}; font: var(--semantic-typography-body-weight) var(--semantic-typography-body-size)/var(--semantic-typography-body-lineHeight) var(--semantic-typography-body-family); letter-spacing: calc(var(--semantic-typography-body-tracking) * 1em); }
    ...
    .collage-nav { display: flex; align-items: center; gap: 1.5rem; padding: 1.1rem clamp(1rem, 4vw, 3rem); background: ${oklchMix(primary, 12, surface)}; }
    ...
    .collage-card { border: 1px solid ${hairline}; border-radius: ${radius}; padding: ${inset}; background: ${oklchMix(surface, 96, primary)}; display: grid; gap: .65rem; align-content: start;${raised} transition: box-shadow ${transition} ${easing}, transform ${transition} ${easing}; }
    ...
    .collage-card p { color: ${oklchMix("currentColor", 78, "transparent")}; }
    .link { color: currentColor; text-decoration: underline; text-underline-offset: .18em; font-weight: 600; }
    .collage-band { margin: clamp(2rem, 5vw, 4rem) calc(clamp(1rem, 4vw, 3rem) * -1) 0; padding: clamp(5rem, 10vw, 8rem) clamp(1rem, 4vw, 3rem); background: ${oklchMix(primary, 10, surface)}; clip-path: polygon(0 6%, 100% 0, 100% 94%, 0 100%); }
